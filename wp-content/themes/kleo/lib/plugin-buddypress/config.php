...
if ( ! function_exists( 'kleo_menu_user_avatar' ) ) {
	...
	function kleo_menu_user_avatar( $item_output, $item, $depth, $args ) {
		...
		if ( is_user_logged_in() ) {
			...
			$output     .= '<a title="' . bp_get_loggedin_user_fullname() . '" class="kleo-bp-user-avatar' . ( $args->has_children && in_array( $depth, array(
					0,
					1
				) ) ? ' js-activated' : '' ) . '" href="' . $url . '" title="' . $attr_title . '">'
...
function kleo_bp_cover_html() {
	...
	if ( bp_is_my_profile() || is_super_admin() ) {
		...
		$output .= '<a href="' . esc_url( bp_displayed_user_domain() . $bp->profile->slug . '/change-cover-image/' ) . '" class="button">' . esc_html__( "Change Cover", 'kleo' ) . '</a>';
		$output .= '</div>';
	}
...
function kleo_bp_group_cover_html() {
	...
	if ( is_user_logged_in() ) {
		...
		if ( groups_is_user_mod( $user_ID, $group_id ) || groups_is_user_admin( $user_ID, $group_id ) ) {
			...
			$message = esc_html__( "Change Cover", 'bpcp' );
			...
			$output .= '<div class="profile-cover-action">';
			$output .= '<a href="' . esc_url( $group_permalink . 'admin/group-cover-image/' ) . '" class="button">' . $message . '</a>';
			$output .= '</div>';
		}
	}
