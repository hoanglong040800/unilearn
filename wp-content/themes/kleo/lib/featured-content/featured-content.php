...
class Featured_Content {
	...
	public static function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'featured_content', array(
			'title'          => esc_html__( 'Featured Content', 'kleo' ),
			'description'    => wp_kses_post( sprintf(
				__( 'Use the <a href="%1$s">"featured" tag</a> to feature your posts. You can change this to a tag of your choice; if no posts match the tag, <a href="%2$s">sticky posts</a> will be displayed instead.', 'kleo' ),
		...
		$wp_customize->add_control( 'featured-content[tag-name]', array(
			'label'    => esc_html__( 'Tag Name', 'kleo' ),
			'section'  => 'featured_content',
			'priority' => 20,
		) );
		$wp_customize->add_control( 'featured-content[hide-tag]', array(
			'label'    => esc_html__( 'Don&rsquo;t display tag on front end.', 'kleo' ),
			'section'  => 'featured_content',
			'type'     => 'checkbox',
			'priority' => 30,
		) );
	...
	public static function enqueue_scripts() {
		wp_enqueue_script( 'featured-content-suggest', get_template_directory_uri() . '/lib/featured-content/admin.js', array(
			'jquery',
			'suggest'
