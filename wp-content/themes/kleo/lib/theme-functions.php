<?php
define( 'SVQ_THEME_VERSION', '4.9.12' );
...
if ( sq_option( 'mobile_user_avatar', 0 ) == 1 ) {
...
add_action( 'init', function () {
	...
	$status = get_option( 'kleo_version_update' );
	...
	update_option( 'kleo_version_update', [ '4_9_1' => true ] );
...
function kleo_custom_redirect( $redirect_to, $requested_redirect_to, $user ) {
	if ( sq_option( 'login_redirect', 'default' ) == 'custom' && sq_option( 'login_redirect_custom', '' ) != '' ) {
		if ( $requested_redirect_to == '' && ! is_wp_error( $user ) ) {
