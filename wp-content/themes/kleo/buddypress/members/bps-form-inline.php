...
foreach ( $F->fields as $f ) {
	...
	switch ( $display ) {
		...
		case 'range-select':
			echo "<label class='sr-only' for='$id'>$f->label</label>\n";
			?>
			<select id="<?php echo $id; ?>" name="<?php echo $name . '[min]'; ?>" class="form-control input-lg">
			...
			<span> - </span>
			<select name="<?php echo $name . '[max]'; ?>" class="form-control input-lg">
				<?php foreach ( $f->options as $key => $label ) { ?>
		...
		case 'distance':
			...
			<div class="<?php echo esc_attr( $class ); ?>">
				<input class="form-control input-lg bps-distance-input" type="number" min="1"
				       name="<?php echo esc_attr( $id ) . '[distance]'; ?>"
				       value="<?php echo esc_attr( $f->value['distance'] ); ?>"
				       placeholder="<?php echo wp_kses_post( $f->label ); ?> <?php echo esc_html( $within ); ?>">

				<select class="form-control input-lg bps-distance-units"
				        name="<?php echo esc_attr( $id ) . '[units]'; ?>">
					<option
						value="km" <?php selected( $f->value['units'], "km" ); ?>><?php echo esc_html( $km ); ?></option>
					<option
						value="miles" <?php selected( $f->value['units'], "miles" ); ?>><?php echo esc_html( $miles ); ?></option>
				</select>
			...
			<div class="<?php echo esc_attr( $class ); ?>">
				...
				<!--<span><?php /*echo esc_html( $of ); */
				?></span>-->
				<input type="text"
				       id="<?php echo esc_attr( $f->unique_id ); ?>"
				       class="form-control input-lg bps-distance-location"
				       name="<?php echo esc_attr( $id ) . '[location]'; ?>"
				       value="<?php echo esc_attr( $f->value['location'] ); ?>"
